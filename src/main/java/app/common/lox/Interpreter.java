...
import static app.common.lox.TokenType.*;
...
public class Interpreter implements Expr.Visitor<Object>, Stmt.Visitor<Void> {
    ...
    Interpreter() {
        ...
        globals.define("clock", new LoxCallable() {
            ...
            @Override
            public Object call(Interpreter interpreter, List<Object> arguments) {
                return (double) System.nanoTime() / (1000 * 1000 * 1000);
            }
        ...
        globals.define("eval", new LoxCallable() {
            @Override
            public int arity() {
                return 1;
            }
            ...
            public Object call(Interpreter interpreter, List<Object> arguments) {
                ...
                if (Lox.hadError) {
                    throw new RuntimeError(tokens.get(0), "Syntax error in eval.");
                }
                Resolver resolver = new Resolver(interpreter);
                resolver.resolve(statements);
                // Stop if there was a syntax error.
                if (Lox.hadError) {
                    throw new RuntimeError(tokens.get(0), "Resolution error in eval.");
                }
    ...
    private void checkNotNil(Token operator, Object left, Object right) {
        if (left == null || right == null) {
            throw new RuntimeError(operator, "Both operands must be not nil.");
        }
    }

    @Override
    public Object visitBinaryExpr(Expr.Binary expr) {
        Object lvalue = evaluate(expr.left);
        Object rvalue = evaluate(expr.right);

        switch (expr.operator.type) {
        case PLUS:
            checkNotNil(expr.operator, lvalue, rvalue);
            ...
            if (lvalue instanceof String || rvalue instanceof String) {
                return stringify(lvalue) + stringify(rvalue);
            }
    ...
    private Object evaluate(Expr expr) {
        // Sea cual sea la expresión que hay adentro, este método la evalúa
        return expr.accept(this);
    }

    // returns false if the object is null or false, else true
